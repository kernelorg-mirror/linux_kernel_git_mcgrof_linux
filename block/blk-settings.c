...
int blk_validate_limits(struct queue_limits *lim)
{
	 ...
	 */
	if (lim->max_premapped_sectors) {
		if (lim->max_dev_sectors)
			lim->max_premapped_sectors = min(lim->max_premapped_sectors,
							 lim->max_dev_sectors);
		lim->max_premapped_sectors = max(lim->max_premapped_sectors,
						 lim->max_sectors);
		lim->max_premapped_sectors = round_down(lim->max_premapped_sectors,
							logical_block_sectors);
	}
