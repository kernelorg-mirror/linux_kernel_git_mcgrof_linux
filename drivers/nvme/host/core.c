		...
		struct queue_limits *lim, bool is_admin)
{
	 ...
	 * to UINT_MAX>>9 when MDTS was absent (max_hw_premapped_sectors left at
	 * UINT_MAX); the block layer keeps it at or above max_sectors.
	 */
	lim->max_hw_premapped_sectors =
		min_t(u32, ctrl->max_hw_premapped_sectors, UINT_MAX >> SECTOR_SHIFT);
...
static int nvme_init_identify(struct nvme_ctrl *ctrl)
{
	...
	/*
	 * The premapped ceiling is the transport's hard maximum (set by the
	 * transport, e.g. NVME_MAX_BYTES for PCI) with MDTS folded in, but
	 * WITHOUT the dma_opt_mapping_size() clamp that max_hw_sectors takes
	 * next: premapped I/O allocates no IOVA per command, so it may reach
	 * the device's real command size. min_not_zero keeps the transport
	 * bound when set and falls back to MDTS otherwise.
	 */
	ctrl->max_hw_premapped_sectors =
		min_not_zero(ctrl->max_hw_premapped_sectors, max_hw_sectors);
	ctrl->max_hw_sectors =
