		...
		struct nvme_ns_head *head, struct nvme_id_ns *id)
{
	 ...
	 * allow block I/O.
	 */
	if (!nvme_lba_shift_supported(head, ctrl)) {
		capacity = 0;
		bs = (1 << 9);
		dev_warn(ctrl->device, "I'm sorry dave, I'm afraid I can't do that\n");
	}
	...
	if (id->nabo == 0) {
		...
		if (id->nsfeat & NVME_NS_FEAT_ATOMICS && id->nawun)
			...
			awun = (1 + ctrl->subsys->awun);
		/* Indicates MDTS can be used */
		if (awun == 0xffff)
			awun_bs = ctrl->max_hw_sectors << SECTOR_SHIFT;
		else
			awun_bs = awun * bs;
	}
